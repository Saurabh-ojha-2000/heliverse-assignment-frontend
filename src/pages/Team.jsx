...
const Team = () => {
  ...
  useEffect(() => {
    const getTeam = async () => {
      const response = await axios.get("https://heliverse-assignment-backend-mu98.onrender.com/api/team/all");
  ...
  const getTeamById = async () => {
    ...
    const response = await axios.get(
      `https://heliverse-assignment-backend-mu98.onrender.com/api/team/${teamId}`
    );
    const data = await response.data;
    setTeams([data]);
  };
  ...
  return (
    <div className="h-full w-full px-4 md:py-3 flex flex-col " style={{backgroundColor:"rgb(222 193 193)"}}>
  ...
  "
          ...
          style={{backgroundColor:"rgb(219 152 152)"}}
        />
        <button
          onClick={getTeamById}
          className="
  text-black flex items-center justify-center
  bg-slate-100  rounded-full h-[30px] w-[30px] focus:outline-none focus:ring-0 focus:ring-opacity-50 placeholder-black"
        >
          <Search />
        </button>
      </div>
      <h1
        className="
        font-bold text-xl md:text-2xl mb-4 text-black
      "
      >
      ...
      text-black gap-y-2 md:gap-2 overflow-y-scroll"
      >
